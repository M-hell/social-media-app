...
const ContentModerator = async (req, res) => {
    ...
    for (let i = 0; i < posts.length; i += batchSize) {
        ...
        for (let post of batch) {
            ...
            const prompt = `description - ${description}. In this description, is there any vulgar words or racist, discriminatory, or offensive language? If it contains such words, just say yes; if no, just say no.`;

            try {
                const result = await generateContentWithRetry(genAI, prompt);
                const responseText = await result.response.text();
                //tuned model check
                // const resultfromTunedModel = await returnGeminiResponseTuned(prompt);
                
                // || resultfromTunedModel.includes("SAFETY") || resultfromTunedModel.toLowerCase().includes("yes")
                ...
                // || resultfromTunedModel.toLowerCase().includes("yes")
